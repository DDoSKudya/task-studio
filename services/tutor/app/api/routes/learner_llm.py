...
from app.domain.model_runtime.status import probe_external, probe_ollama
from fastapi import APIRouter
from studio_contracts.api.tutor_schemas import TutorLlmStatus, TutorLlmTestRequest
...
async def llm_status(
    ...
    _ = user_id
    return await probe_ollama(client, config, ensure=ensure)
...
async def llm_test(
    ...
    if body.provider != "external":
        return await probe_ollama(client, config, ensure=True)

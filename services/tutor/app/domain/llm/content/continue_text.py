...
import httpx
from app.domain.llm.content.prose_dedupe import continuation_is_restart
...
async def complete_json_raw_until_done(
    ...
    *,
    ...
    if extracted is not None and not result.truncated and not _json_looks_open(raw):
        return result
    if extracted is None and "{" not in raw:
        return result
    if extracted is None and not result.truncated and not _json_looks_open(raw):
        return result

...
from .dechrome import apply_dechrome, parse_dechrome_plan
...
async def polish_article_markdown(
    ...
    *,
    ...
    """
    ИИ только анализирует черновик и помечает рекламу/chrome.
    Полный текст правится локально — без сжатия LLM.
    Возвращает (title, cleaned_markdown).
    """
...
async def rescue_article_from_page(
    ...
    *,
    ...
    """
    Запасной путь: детерминированный парсер дал слишком мало текста.
    LLM только вытягивает статью из дампа (без сжатия «для красоты»).
    """

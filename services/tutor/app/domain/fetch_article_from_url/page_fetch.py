...
from .challenge import looks_like_challenge_html
from .thresholds import MIN_ARTICLE_CHARS, SKIP_READER_CHARS
from .url import (
    _BROWSER_HEADERS,
    _FETCH_TIMEOUT,
    _MAX_REDIRECTS,
...
def _extracted_markdown(content_type: str, raw: str) -> str:
    from .html import page_to_article_markdown
...
def _looks_like_page_chrome(markdown: str) -> bool:
    """Шапка Хабра/ленты вместо тела — reader всё ещё нужен."""
...
def _skip_reader(content_type: str, raw: str) -> bool:
    """Reader не нужен только если уже есть солидное тело без chrome."""
...
async def fetch_page(
    ...
    *,
    ...
    """Прямой browser-like GET; при тонком/challenge — reader; берём более длинное тело."""
    ...
    chosen = _pick_richest(direct, via_reader)

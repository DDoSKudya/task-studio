...
from app.domain.fetch_article_from_url.parsing.article_parse import (
    article_response_from_dict,
    parse_article_json,
)
from app.domain.fetch_article_from_url.parsing.clean_llm import (
    polish_article_markdown,
    rescue_article_from_page,
)
...
from app.domain.fetch_article_from_url.sources.page_fetch import fetch_page
...
from app.domain.llm import LlmTarget, course_provider_url, resolve_llm_target
...
async def _resolve_optional_target(
    ...
    except (TutorError, httpx.HTTPError) as exc:
        logger.warning(
            "article ingest: skip optional LLM target (%s)",
...
def _resolve_batch_urls(body: FetchArticlesFromUrlsRequest) -> list[str]:
    ...
    for raw in body.urls:
        ...
        if not text:
            continue
        for url in extract_http_urls(text, limit=MAX_BATCH_URLS):
            if url in seen:
                continue
            seen.add(url)
            collected.append(url)
            if len(collected) >= MAX_BATCH_URLS:
                return collected
    if body.text:
        for url in extract_http_urls(body.text, limit=MAX_BATCH_URLS):
            if url in seen:
                continue
            seen.add(url)
            collected.append(url)

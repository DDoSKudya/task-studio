...
# Заголовки домашки/квиза — не путать с «как это работает на практике».
...
def strip_theory_exercise_sections(markdown: str) -> str:
    """Убирает из уже сгенерированной теории секции домашки/ответов."""
    result = harvest_markdown_exercises(markdown)
    cleaned = result.teaching_markdown
    # На случай заголовков без # (жирный «Задание 1:»).
    cleaned = _strip_inline_exercise_paragraphs(cleaned)
    return re.sub(r"\n{3,}", "\n\n", cleaned).strip()


def exercises_to_prompt_digest(
    exercises: list[HarvestedExercise] | tuple[HarvestedExercise, ...],
    *,
    kinds: set[ExerciseKind] | None = None,
    limit: int = 8,
    char_budget: int = 6_000,
) -> str:
    selected = [item for item in exercises if kinds is None or item.kind in kinds][:limit]
    if not selected:
        return ""
    parts: list[str] = []
    used = 0
    for item in selected:
        block = item.as_prompt_block()
...
def _classify_exercise(title: str, body: str) -> ExerciseKind:
    ...
    if re.search(r"```|^\s{0,3}(?:\$|#)\s+\S", body, re.MULTILINE) and re.search(
        r"(?i)(?:создай|напиши|запусти|выполните|docker|dockerfile|implement|write|run\b)",
        blob,
    ):
        return "practice"

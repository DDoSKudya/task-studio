...
def _finalize_theory_content(text: str) -> str:
    return clean_theory_markdown(strip_theory_exercise_sections(text))
...
def _apply_opening_edit(step: dict[str, object], opening: str, cut: int) -> bool:
    ...
    step["content"] = _finalize_theory_content(f"{revised}\n\n{rest}" if rest else revised)
    return True
...
def _apply_book_polish_edits(
    ...
    for item in edits_raw:
        ...
        if full and _polish_full_content_ok(original, full):
            step["content"] = _finalize_theory_content(full.strip())
            applied += 1
            continue

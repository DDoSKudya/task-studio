...
from .constants import _MAX_ARTICLE_FOR_PROMPT, _MAX_ARTICLE_FOR_PROMPT_COMPACT
...
def _excerpt_balanced(text: str, limit: int) -> str:
    """Head + mid + tail, чтобы середина длинной статьи не выпадала целиком."""
    if limit <= 0:
        return ""
    if len(text) <= limit:
        return text
    if limit < 180:
        return text[:limit]
    # ~40% начало / ~30% середина / ~25% конец (+ разделители).

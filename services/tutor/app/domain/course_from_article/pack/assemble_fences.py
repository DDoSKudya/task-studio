...
def _absorb_leaked_python(lines: list[str], index: int, body: list[str]) -> int:
    while index < len(lines):
        current = lines[index]
        if _FENCE_LINE_RE.match(current.rstrip()):
            ...
            while peek < len(lines) and not lines[peek].strip():
                peek += 1
            if peek < len(lines) and _line_looks_like_python_code(lines[peek], prev_was_code=True):
                index += 1
                continue
            break
...
def _heal_orphaned_python_fences(markdown: str) -> str:
    ...
    while index < len(lines):
        ...
        if fence_match:
            ...
            if index < len(lines):
                index += 1
            index = _absorb_leaked_python(lines, index, body)

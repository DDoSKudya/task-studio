...
def test_use_stream_for_cancel_on_ollama_targets() -> None:
    client_mod = load_service_module("app.domain.llm.transport.client")
...
async def test_complete_chat_result_streams_for_ollama() -> None:
    ...
    async def truncated_stream(*_args, finish_reason_out=None, **_kwargs):
        yield "hel"
        yield "lo"
        if finish_reason_out is not None:
            finish_reason_out.append("length")

    async def silent_stream(*_args, **_kwargs):
        yield "hel"
        yield "lo"

    with patch.object(client_mod, "stream_chat_completion", truncated_stream):
        truncated = await client_mod.complete_chat_result(
            AsyncMock(),
            target,
            system_prompt="sys",
            user_message="hi",
        )
    with patch.object(client_mod, "stream_chat_completion", silent_stream):
        silent = await client_mod.complete_chat_result(
            AsyncMock(),
            target,
            system_prompt="sys",
            user_message="hi",
    ...
    assert silent.finish_reason is None
...
async def test_iter_course_cancel_unloads_ollama(monkeypatch: pytest.MonkeyPatch) -> None:
    pipe = load_service_module("app.domain.course_from_article.workflow.pipeline.pipeline")
    stop = load_service_module("app.domain.ollama.stop")
    stopped: list[str] = []
...
@pytest.mark.asyncio
async def test_iter_course_tutor_error_unloads_ollama(monkeypatch: pytest.MonkeyPatch) -> None:
    pipe = load_service_module("app.domain.course_from_article.workflow.pipeline.pipeline")
    stop = load_service_module("app.domain.ollama.stop")
    stopped: list[str] = []
    ...
    async def failing_body(
        ...
        *,
        ...
        del user_id, body, store, resumed, build_meta
        active_model.clear()
        active_model.append("course-model")
        yield {
            "type": "stage",
        ...
        raise pipe.TutorError(502, "local practice failed for «x»: no usable tasks")
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: (
            MagicMock(),
            SimpleNamespace(build_id=str(uuid.uuid4())),
            SimpleNamespace(),
            False,
        ),
    )
    monkeypatch.setattr(pipe, "_iter_course_from_article_body", failing_body)
    monkeypatch.setattr(pipe, "mark_build_paused", lambda *_a, **_k: None)
    monkeypatch.setattr(pipe, "sync_progress_from_event", lambda *_a, **_k: None)
    monkeypatch.setattr(stop, "stop_ollama_model", fake_stop)

    agen = pipe.iter_course_from_article(
        AsyncMock(),
        SimpleNamespace(ollama_url="http://ollama:11434", ollama_model="fallback"),
        user_id=uuid.uuid4(),
        body=SimpleNamespace(),
    )
    first = await anext(agen)
    assert first["type"] == "stage"
    with pytest.raises(pipe.TutorError, match="local practice failed"):
        await anext(agen)
    assert stopped == ["course-model"]

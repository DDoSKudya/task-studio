...
def _theory_expand_mod():
    ...
    return sys.modules["app.domain.course_from_article.curriculum.theory.theory_expand"]
...
def test_assemble_manifest_nested_phases_valid() -> None:
    ...
    from studio_contracts.packs.manifest import iter_positions
...
async def test_course_stream_emits_ping_while_waiting() -> None:
    keep = load_service_module("app.domain.course_from_article.workflow.events.stream_keepalive")
...
def test_theory_content_message_includes_book_spine() -> None:
    course = load_service_module("app.domain.course_from_article")
    ...
    msg = course._theory_content_user_message(
        chapters[1],
        body,
        ["learn"],
        chapters=chapters,
...
def test_locale_prompt_ignores_source_language() -> None:
    locale = load_service_module("app.domain.course_from_article.curriculum.outline.course_locale")
    ...
    assert "does not matter" in locale.locale_line("ru")
    assert "ANY language" in locale.locale_line("en")
    assert locale.catalog_text_mismatches_locale("Path operations", "ru")
    assert locale.catalog_text_mismatches_locale("Path operations and FastAPI routing basics", "ru")
    assert not locale.catalog_text_mismatches_locale(
        "Операции пути и маршрутизация приложения FastAPI", "ru"
    )
    english_chapters = [
        {"id": "t1", "title": "Path operations", "objective": "Learn routing basics"}
    ]
    russian_chapters = [
        {
            "id": "t1",
    ...
    assert locale.catalog_needs_locale_rewrite(english_chapters, [], "ru")
    assert not locale.catalog_needs_locale_rewrite(russian_chapters, [], "ru")
    assert not locale.catalog_needs_locale_rewrite(english_chapters, [], "en")


def test_course_from_article_skills_skip_removed_consistency() -> None:
    prompts = load_service_module("app.domain.prompt_compose.facade")
    req = prompts.PromptRequest(
        mode="course_from_article",
        phase=None,
    ...
    skills = prompts.skills_for(req)
    assert "article-consistency" not in skills
    assert "course-stage-json" in skills


def test_course_from_article_skills_include_quality_gate() -> None:
    prompts = load_service_module("app.domain.prompt_compose.facade")
    req = prompts.PromptRequest(
        mode="course_from_article",
        phase=None,
    ...
    skills = prompts.skills_for(req)
    assert "chapter-quality-gate" in skills
    assert "anti-hallucination-source" in skills
...
def test_course_skills_include_domain_overlay_and_practice_drill() -> None:
    prompts = load_service_module("app.domain.prompt_compose.facade")
    theory = prompts.PromptRequest(
        mode="course_from_article",
        phase=None,
        step_kind="theory",
        step_title="theory",
    ...
    tasks = prompts.PromptRequest(
        ...
        compact=False,
        sql_aware=False,
        course_profile="language_learning",
    )
    theory_skills = prompts.skills_for(theory)
    assert "domain-programming" in theory_skills
    assert "instructional-design" in theory_skills
    assert "expand-dense-prose" in theory_skills
    code_skills = prompts.skills_for(code)
    assert "practice-as-drill" in code_skills
    assert "code-task-ladder" in code_skills
    assert "domain-humanities" in code_skills
    task_skills = prompts.skills_for(tasks)
    assert "practice-as-drill" in task_skills
    assert "open-task-ladder" in task_skills
    assert "domain-language-learning" in task_skills
    untitled = prompts.PromptRequest(
        mode="course_from_article",
        phase=None,
        step_kind="theory",
        step_title="theory",
    ...
    assert "domain-general" in prompts.skills_for(untitled)
...
async def test_generate_course_from_article_mocked_stages(tutor_config) -> None:
    ...
    stage_responses = {
        ...
        "theory": {
            ...
            "content": (
                "Когда путь — это строка, легко ошибиться с разделителями. "
                "Path оборачивает файловые пути и позволяет склеивать части через `/`. "
                "`.name` — последний компонент, `.stem` — имя без суффикса. "
                "Ниже разберём join, resolve и относительные пути на примерах из статьи. "
                + ("Path joins filesystem parts safely. " * 12)
            ),
        },
        "quizzes": {
            "quizzes": [
                {
                    "id": f"quiz-{i}",
                    "title": f"Q{i}",
                    "question": (f"What does Path provide for filesystem paths in check {i}?"),
                    "choices": [
    ...
    async def fake_stage_json(*_args, stage: str, user_message: str = "", **_kwargs):
        ...
        if stage == "polish":
            return {"edits": []}
        if stage == "quality":
            return {"ok": True, "score": 0.9, "issues": [], "must_fix": []}
    ...
    with (
        ...
        patch(
            "app.domain.course_from_article.quality.chapter_quality.complete_text_until_done",
            new=AsyncMock(return_value=str(stage_responses["theory"]["content"])),
        ),
    ):
...
async def test_external_theory_mid_chapters_preserve_order(tutor_config) -> None:
    ...
    async def fake_stage_json(*_args, stage: str, user_message: str = "", **_kwargs):
        ...
        if stage == "theory":
            ...
            return {
                ...
                "title": title,
                "content": (
                    f"Prose for {title}. Path wraps filesystem paths and joins parts. "
                    + ("Path joins filesystem parts safely. " * 14)
                ),
            }
        if stage == "quizzes":
            index = 1
                ...
                "quizzes": [
                    {
                        ...
                        "title": f"Q{index}",
                        "question": (
                            f"What does Path provide for filesystem paths in check {index}?"
                        ),
                        "choices": [
                            "Join path parts safely",
                            "Only print the CWD",
    ...
    with (
        ...
        patch.object(
            _theory_expand_mod(),
            "complete_text_until_done",
            new=AsyncMock(return_value="Path helps with files in theory chapters. " * 20),
        ),
        patch(
            "app.domain.course_from_article.quality.chapter_quality.complete_text_until_done",
            new=AsyncMock(return_value="Path helps with files in theory chapters. " * 20),
...
def test_ladder_levels_round_robin() -> None:
    practice = load_service_module("app.domain.course_from_article.practice.practice_generate")
...
def test_coerce_quiz_answer_accepts_string_indices() -> None:
    practice = load_service_module("app.domain.course_from_article.practice.normalize_practice")
        ...
        [
            {
                "id": "quiz-1",
                "title": "Q",
                "question": "Which statement matches the article best?",
                "choices": ["a", "b", "c", "d"],
                "answer": "1",
            }
        ],
...
def test_normalize_quizzes_rejects_incomplete_choices() -> None:
    practice = load_service_module("app.domain.course_from_article.practice.normalize_practice")
    quizzes = practice._normalize_quizzes(
        [
            {
...
def test_normalize_code_tasks_rejects_tiny_template() -> None:
    practice = load_service_module("app.domain.course_from_article.practice.normalize_practice")
    tasks = practice._normalize_code_tasks(
        [
            {
                "id": "tiny",
                "title": "Incomplete starter",
                "content": "Complete the requested operation using the supplied runtime.",
                "template": "pass",
                "tests": [],
                "checker": "llm",
            }
...
def test_weak_code_template_is_converted_to_open_task() -> None:
    practice = load_service_module("app.domain.course_from_article.practice.practice_generate")
    warnings: list[str] = []

    task = practice._ensure_deliverable_practice(
        {
            "id": "code-easy",
            "kind": "code",
            "level": "easy",
            "title": "Apply the operation",
            "content": "Use the runtime syntax to produce the requested observable result.",
            "runtime": "custom",
            "template": "// TODO",
            "tests": [],
            "checker": "llm",
    ...
    assert "template" not in task
    assert task["checker"] == "llm"
    assert any("converted to open task" in warning for warning in warnings)
    ...
    async def fake_stage_json(*_args, stage: str, user_message: str = "", **_kwargs):
        ...
        return {
            "quiz": {
                "id": f"quiz-{index}",
                "title": f"Q{index}",
                "question": (f"What does Path provide for filesystem paths in check {index}?"),
                "choices": [
                    "Join path parts safely",
                    "Only print the CWD",
                    "Delete files by default",
    ...
    async def fake_stage_json(*_args, **_kwargs):
        ...
        return {
            "quiz": {
                "id": f"quiz-{calls['n']}",
                "title": "Ok",
                "question": (f"What does Path provide for filesystem paths in check {calls['n']}?"),
                "choices": [
                    "Join path parts safely",
                    "Only print the CWD",
                    "Delete files by default",

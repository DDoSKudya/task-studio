...
def _load_retry_module():
    module_path = (
        Path(__file__).resolve().parents[3] / "app" / "domain" / "llm" / "transport" / "retry.py"
    )
...
@pytest.mark.asyncio
async def test_retry_until_mapping_recovers_after_empty_and_prose(
    ...
    async def _parse(raw: str) -> dict[str, object] | None:
        return {"a": 1} if raw.startswith("{") else None

    async def _no_sleep(_delay: float) -> None:
        return None

    monkeypatch.setattr(retry.asyncio, "sleep", _no_sleep)
    parsed = await retry.retry_until_mapping(_generate, _parse, retries=2)
    assert parsed == {"a": 1}
    assert not raws


@pytest.mark.asyncio
async def test_retry_until_mapping_raises_after_retries(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    retry = _load_retry_module()
    ...
    async def _parse(_raw: str) -> dict[str, object] | None:
        return None
    ...
    with pytest.raises(ValueError, match="expected JSON object"):
        await retry.retry_until_mapping(_generate, _parse, retries=2)


@pytest.mark.asyncio
async def test_retry_until_mapping_does_not_retry_unauthorized() -> None:
    retry = _load_retry_module()
    ...
    with pytest.raises(httpx.HTTPStatusError):
        await retry.retry_until_mapping(_generate, _parse, retries=3)


def test_empty_llm_content_is_transient() -> None:
    retry = _load_retry_module()
    assert retry.is_transient_llm_error(ValueError("empty LLM content"))
    assert retry.is_transient_llm_error(ValueError("llama runner process has terminated"))
    assert retry.is_transient_llm_error(ValueError("expected JSON object"))
    assert retry.is_json_shape_error(ValueError("expected JSON object"))
    assert retry.is_json_shape_error(ValueError("course stage quizzes returned invalid JSON"))
    assert not retry.is_transient_llm_error(ValueError("invalid API key"))

#!/usr/bin/env python3
"""Снимки реального UI для docs/DEVELOPERS.md (PNG через Playwright).

Playwright не умеет надёжный «скриншот в SVG» страницы (шрифты, canvas, тени).
Для документации кладём PNG в docs/assets/ui/.

Требования:
  - стек поднят (http://localhost отвечает)
  - pip install playwright && playwright install chromium

Пример:
  python scripts/docs-capture-ui.py
...
def _redact_secrets(page: _Page) -> None:
    """Очищает секреты и маскирует почту, инициалы и пользовательские названия курсов."""
    page.evaluate(
        """() => {

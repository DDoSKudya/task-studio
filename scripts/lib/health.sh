#!/usr/bin/env bash
# UI readiness: free host port + fingerprint (not bare GET / on :80).
...
resolve_ts_http_port() {
  ...
  if [[ "$existing" =~ ^[0-9]+$ ]]; then
    # Уже наш UI на этом порту — оставляем (в т.ч. если loopback «занят» нами).
    if ts_ui_fingerprint_ok "$(ts_format_ui_probe_url "$existing")"; then
      ts_sync_ui_endpoint_vars "$existing"
      printf '%s\n' "$existing"
      return 0
    fi
    # Свободен на 127.0.0.1 — можно публиковать nginx сюда.
    if ! ts_loopback_port_busy "$existing"; then
      ts_sync_ui_endpoint_vars "$existing"
      printf '%s\n' "$existing"
      return 0
    fi
    # Занят чужим слушателем без отпечатка Task Studio — ищем другой.
  fi

...
if (-not (Get-Command Resolve-TsHttpPort -ErrorAction SilentlyContinue)) {
  . (Join-Path $PSScriptRoot '../health/Health.ps1')
}
...
$script:OllamaModel = if ($env:OLLAMA_MODEL) { $env:OLLAMA_MODEL } else { "qwen2.5:7b" }
...
function Get-TsOllamaProfileModels {
  param([string]$Profile = "cpu-balanced")

  switch ($Profile) {
    "cpu-light" {
      return @("qwen2.5:1.5b", "qwen2.5:3b", "qwen2.5:3b", "nomic-embed-text")
    }
    "cpu-balanced" {
      return @("qwen2.5:3b", "qwen2.5:3b", "qwen2.5:3b", "nomic-embed-text")
    }
    "gpu-light" {
      return @("qwen2.5:7b", "qwen2.5:7b", "qwen2.5:7b", "nomic-embed-text")
    }
    "gpu-balanced" {
      return @("qwen2.5:7b", "qwen2.5:7b", "qwen2.5:7b", "nomic-embed-text")
    }
...
function Set-TsOllamaProfile {
  ...
  $bundle = @(Get-TsOllamaProfileModels -Profile $profile)
  $chat = $bundle[0]
  $course = $bundle[1]
  $polish = $bundle[2]
  $embed = $bundle[3]
  ...
  $env:OLLAMA_MODEL = $course
  $env:OLLAMA_MODEL_CHAT = $chat
  $env:OLLAMA_MODEL_COURSE = $course
  $env:OLLAMA_MODEL_POLISH = $polish
  $env:OLLAMA_MODEL_EMBED = $embed
  if ($null -ne $vramGb) {
  ...
  if (Test-Path .env) {
    ...
    Set-TsEnvUpsert -Key "OLLAMA_MODEL" -Value $course
    Set-TsEnvUpsert -Key "OLLAMA_MODEL_CHAT" -Value $chat
    Set-TsEnvUpsert -Key "OLLAMA_MODEL_COURSE" -Value $course
    Set-TsEnvUpsert -Key "OLLAMA_MODEL_POLISH" -Value $polish
    Set-TsEnvUpsert -Key "OLLAMA_MODEL_EMBED" -Value $embed
  }
}
...
function Prepare-TsDirs {
  @(
    ...
    "data/logs", "data/course-gold", "data/course-adapters", "data/course-builds"
  ) | ForEach-Object { New-Item -ItemType Directory -Force -Path $_ | Out-Null }
}

function Reset-TsComposeImages {
  param([object[]]$ProfileArgs)

  [void](Invoke-TsCompose @($ProfileArgs + @("down", "--rmi", "all", "--remove-orphans")))
  Invoke-TsCompose @($ProfileArgs + @("build", "--no-cache"))
...
function Invoke-TsInstall {
  ...
  if ($repair) {
    Enter-TsProgressStage -Plan $plan -Id "build" -Status (Get-TsText status_reset_images)
    Reset-TsComposeImages -ProfileArgs $profileArgs
  } else {
  ...
  Set-TsOllamaProfile
  $seen = @{}
  foreach ($model in @(Get-TsOllamaProfileModels -Profile $env:OLLAMA_PROFILE)) {
...
function Invoke-TsUpdate {
  ...
  Enter-TsProgressStage -Plan $plan -Id "rebuild" -Status (Get-TsText status_reset_images)
  if (-not (Test-Path ".env")) {

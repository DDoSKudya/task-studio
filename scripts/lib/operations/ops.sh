...
ops_reset_compose_images() {

  local profile_args="$1"
  # shellcheck disable=SC2086
  ops_compose $profile_args down --rmi all --remove-orphans || true
  # shellcheck disable=SC2086
  ops_compose $profile_args build --no-cache
}
...
ops_ollama_profile_models() {

  local profile="${1:-cpu-balanced}"
  local chat course polish embed
  case "$profile" in
    cpu-light)
      chat="qwen2.5:1.5b"; course="qwen2.5:3b"; polish="qwen2.5:3b" ;;
    cpu-balanced)
      chat="qwen2.5:3b"; course="qwen2.5:3b"; polish="qwen2.5:3b" ;;
    gpu-light)
      chat="qwen2.5:7b"; course="qwen2.5:7b"; polish="qwen2.5:7b" ;;
    gpu-balanced)
      chat="qwen2.5:7b"; course="qwen2.5:7b"; polish="qwen2.5:7b" ;;
    *)
      chat="qwen2.5:3b"; course="qwen2.5:3b"; polish="qwen2.5:3b" ;;
  ...
  printf '%s\n' "$chat" "$course" "$polish" "$embed"
}

ops_configure_ollama_profile() {
  local accel profile vram_gb chat course polish embed
  accel="$(ops_detect_ollama_accelerator)"
  ...
  # shellcheck disable=SC2034
  {
    ...
    read -r polish
    read -r embed
  } < <(ops_ollama_profile_models "$profile")
  export OLLAMA_MODEL_CHAT="$chat"
  export OLLAMA_MODEL_COURSE="$course"
  export OLLAMA_MODEL_POLISH="$polish"
  export OLLAMA_MODEL_EMBED="$embed"
  export OLLAMA_MODEL="$course"
  if [[ -f .env ]]; then
    ...
    ops_env_upsert OLLAMA_MODEL "$course"
    ops_env_upsert OLLAMA_MODEL_CHAT "$chat"
    ops_env_upsert OLLAMA_MODEL_COURSE "$course"
    ops_env_upsert OLLAMA_MODEL_POLISH "$polish"
    ops_env_upsert OLLAMA_MODEL_EMBED "$embed"
  fi
}
...
ops_pull_ollama() {
  ...
  done < <(ops_ollama_profile_models "${OLLAMA_PROFILE:-cpu-balanced}")
}
...
ops_install() {
  ...
  if [[ "$repair" -eq 1 ]]; then
    ts_prog_enter build "$(ts_t status_reset_images)"
    if ! ops_reset_compose_images "$profile_args"; then
...
ops_update_apply() {
  ...
  ts_prog_enter rebuild "$(ts_t status_reset_images)"
  if [[ ! -f .env ]]; then

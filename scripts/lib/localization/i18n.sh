...
ts_t() {
  ...
  case "$key" in
    ...
    status_reset_images) en="Removing Docker images and rebuilding (course data on disk is kept)…"; ru="Удаляем образы Docker и собираем заново (данные курсов на диске сохраняются)…" ;;

...
const props = defineProps<{
  ...
  enabledStages?: CourseStageName[]
  stageLabels?: Partial<Record<CourseStageName, string>>
...
const displayProgress = computed(() => {
  if (!props.error && (props.done || props.progress >= 0.999 || props.currentStage === 'done')) {
    return 1
  }
  ...
  >
    ...
    <ol class="course-progress-stages" :data-count="STAGES.length">
      ...
      >
        ...
        <span class="course-progress-stage-label">{{
          stageLabels?.[stage.id] ?? t(`courseBuild.stages.${stage.id}`)
        }}</span>
        <span

import type { AdapterInfo } from '~/composables/search/useSearch'
...
export function useSettingsPage() {
  ...
  function persistActiveProviderDraft(mode: TutorProviderMode = tutorProviderMode.value) {
  ...
  watch(tutorProviderMode, (next, prev) => {
    ...
    if (prev) {
      persistActiveProviderDraft(prev)
    }
  ...
  const ollamaCourseModelLabel = computed(() => {
    if (modelsLoadPending.value || llmStatusPending.value) {
      return t('settings.tutor.modelManagedPending')
    }
    const status = llmStatus.value
    if (!status?.ok) {
      return t('settings.tutor.ollamaModelUnavailable')
    }
    return (
      status.active_course_model?.trim()
      || status.default_model?.trim()
      || t('settings.tutor.modelManagedPending')
    )
  })
  ...
  function applyLoadedModels(status: Awaited<ReturnType<typeof getLlmStatus>>) {
    ...
    const managed =
      ...
      && Boolean(status.model_managed || status.active_course_model)
    if (managed) {
      const nextModel = status.ok
        ? (status.active_course_model || status.default_model || 'auto')
        : ''
      tutor.value.model = nextModel

      if (baseline.value.tutor.providerMode === 'ollama') {
        baseline.value = {
          ...baseline.value,
          tutor: {
            ...baseline.value.tutor,
            model: nextModel,
  ...
  async function loadAvailableModels(options?: { quiet?: boolean }) {
    ...
    try {
      if (tutorProviderMode.value === 'ollama') {
        ...
        try {
          applyLoadedModels(await getLlmStatus({ ensure: true }))
        } catch {
          void 0
        }
      } else {
  ...
  async function verifyTutorBeforeSave(): Promise<boolean> {
    ...
    if (tutorProviderMode.value === 'ollama') {
      ...
      try {
        ...
        if (!ok || !llmStatus.value?.ok) {
          toasts.error(llmStatus.value?.detail || t('settings.tutor.statusUnavailable'))
          return false
        }
        tutor.value.model =
          llmStatus.value.active_course_model
          || llmStatus.value.default_model
          || tutor.value.model
          || 'auto'
        return true
      } finally {
